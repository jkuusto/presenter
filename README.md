...
```
#### How to Fix It
Remove the `|safe` filter when rendering comments. This removes the bypassing of Django's built-in backend mechanisms which ensure that user-generated content is properly escaped:
```
<!-- detail.html -->
<ul>
...
The design of the app dictates that only logged-in users can vote and view the vote results. Only an authenticated user can vote, after which they are automatically redirected to the poll question's results page. There is no direct navigational link to view the results without voting.
...
#### How to Fix It
Add a `@method_decorator(login_required, name='dispatch')` decorator to the ResultsView class (see source code link above). This ensures that access to each results view is granted only when a user is logged in, fixing the broken access control flaw.
